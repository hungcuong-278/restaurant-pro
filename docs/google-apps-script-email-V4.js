// ============================================================
//  ✅ PHIÊN BẢN MỚI NHẤT ĐÃ HOÀN THIỆN (FIX LỖI GỬI EMAIL)
//
//  ⚠️ YÊU CẦU QUAN TRỌNG KHI DEPLOY:
//  - Bạn PHẢI mở trình duyệt ẩn danh (Incognito).
//  - Đăng nhập DUY NHẤT tài khoản no-reply@hoinghitamthanhoctoanquoc.com
//  - Tạo Google Sheet và copy dán toàn bộ script này vào Apps Script.
...
const CONFIG = {
  ...
  // ✅ ĐÃ CẬP NHẬT: Địa chỉ khi người nhận bấm "Trả lời" email xác nhận
  REPLY_TO     : 'bantochuc@hoinghitamthanhoctoanquoc.com',

  // ✅ ĐÃ CẬP NHẬT: Email nhận thông báo khi có đăng ký mới
  ADMIN_EMAIL  : 'bantochuc@hoinghitamthanhoctoanquoc.com', 
  ...
  MIN_FILL_MS  : 0,   // ms tối thiểu điền form — chặn bot
};
...
function doGet(e) {
  try {
    ...
    // Thực hiện gửi email với try-catch để bắt lỗi
    let emailStatus = '';
    try {
      sendConfirmation(data);
    } catch (e) {
      emailStatus += 'Lỗi xác nhận: ' + e.message + ' ';
    }

    try {
      sendAdminAlert(data);
    } catch (e) {
      emailStatus += 'Lỗi Admin: ' + e.message;
    }

    data._emailStatus = emailStatus === '' ? 'Đã gửi Email' : emailStatus;
    
    // Lưu vào sheet (bao gồm cả trạng thái email)
    saveToSheet(data);
...
function saveToSheet(data) {
  ...
  if (!sheet) {
    ...
    const headers = [
      ...
      'Email', 'Số điện thoại', 'Đơn vị', 'Chức vụ', 'Nội dung tham dự', 'Trạng thái Email'
    ];
  ...
  // STT chính xác — hàng 1 là header nên STT = lastRow - 1, khi chuẩn bị thêm dòng mới là đúng bằng lastRow
...
<table width="600" cellpadding="0" cellspacing="0"
  ...
  // Dùng MailApp thay vì GmailApp sẽ ít bị lỗi phân quyền hơn cho các tài khoản Google Workspace
  MailApp.sendEmail({
    to       : data.Email,
    subject  : subject,
    htmlBody : html,
...
// CORS header để fetch từ trình duyệt không bị chặn
 ...
 * Kết quả: Sẽ có 1 email gửi cho người đăng ký test (Email bên dưới), 
 * và 1 email gửi cho ADMIN_EMAIL.
 */
function testScript() {
  const fakeData = {
    Ho_Ten           : 'Nguyễn Văn Test',
    Email            : 'bantochuc@hoinghitamthanhoctoanquoc.com',   // gửi về chính mình để kiểm tra
  ...
  try {
    ...
    Logger.log('✅ TEST THÀNH CÔNG — Vui lòng kiểm tra email của bạn.');
...
function deployInstructions() {
  const msg = [
    '📋 HƯỚNG DẪN DEPLOY:',
    '',
    '⚠️ QUAN TRỌNG: Hãy đảm bảo bạn chỉ đăng nhập DUY NHẤT tài khoản no-reply@hoinghitamthanhoctoanquoc.com',
    '   trong một cửa sổ ẩn danh (Incognito) để tránh bị Google nhầm lẫn quyền với tài khoản cá nhân.',
    '',
    '1. Chạy testScript() → kiểm tra email về đúng không',
    '2. Vào menu: Deploy > New deployment',
    '3. Chọn type: Web App',
    '4. Execute as: Me  ← "Me" lúc này chính là tài khoản đang deploy.',
    '5. Who has access: Anyone',
    '6. Deploy → Copy Web App URL',
    '7. Dán URL vào landing page (trong file App.jsx)',
  ].join('\n');
