...
const CONFIG = {
  ...
  ADMIN_EMAIL  : 'noreply@hoinghitamthanhoctoanquoc.com', // email nhận thông báo đăng ký mới
...
function doGet(e) {
  try {
    ...
    // Validate bắt buộc
    const required = { Ho_Ten: 'Họ tên', Email: 'Email', So_Dien_Thoai: 'Số điện thoại', Don_Vi: 'Đơn vị' };
...
function saveToSheet(data) {
  ...
  if (!sheet) {
    ...
    const headers = ['STT','Thời gian','Múi giờ','Họ và tên','Ngày sinh',
  ...
  const stt     = Math.max(sheet.getLastRow(), 1); // hàng 1 = header → STT bắt đầu từ 1
...
function sendConfirmation(data) {
  ...
  const infoRows = rows.map(([label, val]) => `
    <tr>
      ...
      <td style="padding:6px 0;color:#111827;font-size:14px;font-weight:500;">${val}</td>
    </tr>`).join('');
...
<table width="600" cellpadding="0" cellspacing="0"
  ...
  <tr><td style="padding:40px;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0"
      style="background:#F0F7F4;border-radius:8px;border-left:4px solid #0D3C1F;
             margin-bottom:24px;">
    ...
    <p style="margin:0 0 20px;font-size:14px;color:#6B7280;line-height:1.7;">
      Nếu có thắc mắc, vui lòng liên hệ Ban Tổ chức qua địa chỉ email này.</p>
